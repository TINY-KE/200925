...
{
        resultFlag = 1;
        // std::cout <<  "\rResult: [ " << std::string(string) <<" ]" <<std::endl;
       printf("\rResult: [ %s ]", string);
        if (is_over)
                putchar('\n');
}

static char *g_result = NULL;   //语音识别的结果
...
{
        if (g_result)  //如果g_result里有东西 就清空
        {
                free(g_result);
        }
...
//.  1 为什么要订阅这个信号
        ...
        while (ros::ok())
        {
                ...
                if (wakeupFlag)
                {
                        ROS_INFO("Wakeup...");
                        int ret = MSP_SUCCESS;
                        const char *login_params = "appid = 5f5adf7b, work_dir = .";
