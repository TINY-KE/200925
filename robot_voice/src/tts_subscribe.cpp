...
void voiceWordsCallback(const std_msgs::String::ConstPtr& msg)
{
    ...
    unlink("/tmp/cmd");  
    mkfifo("/tmp/cmd", 0777);
    popen("mplayer -quiet -slave -input file=/tmp/cmd 'tts_sample.wav'","r");  //TTS需要安装mplayer才能出声，
    sleep(3);
}
...
//1
    int         ret                  = MSP_SUCCESS;
    const char* login_params         = "appid = 5f5adf7b, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
    /*
    * rdn:           合成音频数字发音方式
    * volume:        合成音频的音量
    * pitch:         合成音频的音调
